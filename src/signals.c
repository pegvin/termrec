...
static void do_backtrace(int sig, siginfo_t *siginfo, void *context) {
	void *callstack[128];
	char **strs;
	int frames;
	...
	if (ioctl(STDOUT_FILENO, TIOCSWINSZ, &rwin) == -1) perror("ioctl(TIOCSWINSZ)");
...
static void handle_sigwinch(int sig) {
	 ...
	 */
	if (ioctl(STDIN_FILENO, TIOCGWINSZ, &rwin) == -1) die("ioctl(TIOCGWINSZ)");
	...
	if (win.ws_row != minrow || win.ws_col != mincol) {
		...
		if (ioctl(masterfd, TIOCSWINSZ, &win) == -1) die("ioctl(TIOCSWINSZ)");
	}
}
